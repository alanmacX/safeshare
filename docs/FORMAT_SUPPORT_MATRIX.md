...
| 动态照片 | 完整支持元数据；暂不支持视觉编辑 | 分离静态图与视频；封面沿用原格式清理，视频定长清除 `moov` 隐私值并保留全部轨道/样本/锚点，再组合为 MovingPhoto | 已实现拆分、双组件复检、重组和成对清理；待 API 24 真机相册保存/分享回读 |
...
| 动态照片 | ✓ 双组件扫描/清理 | — | — | — | ✓ 代码保持封面格式、视频轨道/样本/锚点并重组；待真机回读 |
